# SL Data Center - sl-trino

## Start trino cluster (Locally)
1. Generate self-signed certification

You can generate a self-signed certificate using the following command, please don't use it in your online environment
```
mkdir ./local/init/trino-elt/certs/

keytool -genkeypair -alias trino -keyalg RSA -keystore ./local/init/trino-elt/certs/keystore.jks \
...
keytool -exportcert -file ./local/init/trino-elt/certs/trino.cer -alias trino -keystore ./local/init/trino-elt/certs/keystore.jks -storepass password

keytool -import -v -trustcacerts -alias trino_trust -file ./local/init/trino-elt/certs/trino.cer -keystore ./local/init/trino-elt/certs/truststore.jks -storepass password -keypass password -noprompt

keytool -keystore ./local/init/trino-elt/certs/keystore.jks -exportcert -alias trino -storepass password| openssl x509 -inform der -text

keytool -importkeystore -srckeystore ./local/init/trino-elt/certs/keystore.jks -destkeystore ./local/init/trino-elt/certs/trino.p12 -srcstoretype jks -deststoretype pkcs12 -srcstorepass password -deststorepass password

openssl pkcs12 -in ./local/init/trino-elt/certs/trino.p12 -out ./local/init/trino-elt/certs/trino.pem -passin pass:password -passout pass:password

openssl x509 -in ./local/init/trino-elt/certs/trino.cer -inform DER -out ./local/init/trino-elt/certs/trino.crt
```

2. Make sure .env is well set
   - Refer to ./local/init/trino-elt/.env.example
...
3. Run containers
```
make build profiles=trino-elt
make start profiles=trino-elt
```
...
## user group is set to : ./local/init/trino-elt/coordinator/group.txt

# login trino
./trino --server https://localhost:8443 --truststore-path ./local/init/trino-elt/certs/truststore.jks --truststore-password=password --user <user_name> --password
...
## Set new data source
```
# set the coordinator catalog folders (data-center-infra/local/init/trino-elt/coordinator/catalog)
# set the worker catalog folders (data-center-infra/local/init/trino-elt/worker*/catalog)
# set access_rule.json (./local/init/trino-elt/coordinator/access_rules.json)
    {
        "group": "admin",
